...
//            return;
    ...
    private static boolean checkIfNotNeedAdjust(Context context) {
        ...
        return Math.min(point.y,point.x) == DESIGN_WIDTH && density == DESIGN_SCALE&&Math.max(point.y,point.x)==DESIGN_HEIGHT;
    }
    ...
    private static void AdjustApplicationDensity(Context context) {
        ...
        if (type == TYPE_FONT) {
            ...
            if (Constance.DEBUGTAG)
                Log.i(Constance.DEBUG, "AdjustApplicationDensity: " + mFontScale + "scaledDensity");
            displayMetrics.xdpi = DESIGN_SCALE*72f * point.x / DESIGN_WIDTH;
            ...
            DisplayMetrics metrics = getMetricsOnMiui(resources);
            if (metrics != null) {
                metrics.xdpi =  DESIGN_SCALE*72f * point.x / DESIGN_WIDTH;
                metrics.scaledDensity = mFontScale;
            }
        }
    }
