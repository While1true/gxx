...
public class SuperFileView2 extends FrameLayout implements TbsReaderView.ReaderCallback {
    ...
    File file=new File(Environment.getExternalStorageDirectory(),"gjjcache");
    ...
    public void displayFile(File mFile) {
        ...
        if (mFile != null && !TextUtils.isEmpty(mFile.toString())) {
            ...
            String bsReaderTemp = file.getAbsolutePath()+"/TbsReaderTemp";
            File bsReaderTempFile =new File(bsReaderTemp);

            if (!bsReaderTempFile.exists()) {
                TLog.d("准备创建"+bsReaderTemp);
                boolean mkdir = bsReaderTempFile.mkdir();
                if(!mkdir){
                    TLog.e("创建"+bsReaderTemp+"失败！！！！！");
                }
            }
